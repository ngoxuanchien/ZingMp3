...
import zingmp3.hcmus.playlistservice.converter.PlaylistConverter;
...
//        return result;
    ...
    public List<PlaylistDTO> getPlaylist(UUID userId) {

        return playlistRepository.findAllByCreatedBy(userId)
                .stream()
...
//        playlistRepository.save(playlist);
    ...
    public PlaylistDTO create(PlaylistDTO playlistDTO) {
        Playlist newPlaylist = playlistConverter.reverseConvert(playlistDTO);
        return playlistConverter
                .convert(playlistRepository.save(newPlaylist));
    }
