...
//            throw e;
    ...
    @Override
    public ArtistResponseGrpc getByIdGrpc(UUID uuid) {
        try {
            var request = ArtistRequestGrpc.newBuilder().setId(uuid.toString()).build();
            return artistClient.getById(request);
...
//            }
//            throw e;
            return ArtistResponseGrpc.newBuilder().setId(uuid.toString()).build();
        }
    }

    @Override
    public List<ArtistResponseGrpc> getAllByIdGrpc(List<UUID> uuids) {
        return uuids.stream()
                .map(this::getByIdGrpc)
                .toList();
    }
}
