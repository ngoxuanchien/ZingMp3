...
import hcmus.zingmp3.music_service.song.service.SongServiceImpl;
...
import static hcmus.zingmp3.music_service.song.SongDataForTest.*;
...
class SongServiceTest {
    ...
    void SongService_CreateSong_ReturnSongResponse() {
        SongRequest songRequest = SONG_REQUESTS.getFirst();
        Song songBeforeSave = SONG_BEFORE_SAVE.getFirst();
        Song songAfterSave = SONG_AFTER_SAVE.getFirst();
        SongResponse songResponse = SONG_RESPONSES.getFirst();
    ...
    void SongService_GetSongById_ReturnSongDto() {
        UUID id = SONG_IDS.getFirst();
        Song song = SONG_AFTER_SAVE.getFirst();
        SongResponse songResponse = SONG_RESPONSES.getFirst();
    ...
    void SongService_GetAllSongs_ReturnListSongResponse() {
        ...
        List<Song> songList = SONG_AFTER_SAVE;
        Page<Song> songPage = new PageImpl<>(songList, pageable, songList.size());
        List<SongResponse> songResponseList = SONG_RESPONSES;

        when(songRepository.findAll(pageable)).thenReturn(songPage);
        when(songMapper.toDTO(SONG_AFTER_SAVE)).thenReturn(songResponseList);
